...
| **toolkit config** | The persistent user-level configuration file for pi-web-toolkit runtime options and discovered external CLI paths, used when environment variables are not set. Its default location is `${XDG_CONFIG_HOME:-~/.config}/pi-web-toolkit/config.json`; schema, defaults, precedence, validation, and writes are owned by the shared config core. |
...
| **browser-action-language** | Shared `web_browse` action semantics module; owns validation, human-readable steps, local agent-browser command planning, and Firecrawl interact prompt planning. |
...
| **page-extraction** | Shared page-read module for `web_fetch` and `web_batch_fetch`; owns Scrapling invocation, local temp files, cleanup, byte accounting, preview generation, and page-read output fallback. |
| **output-sink** | Unified output handling module: truncation policy, temp-file fallback, and truncation notice formatting. Used by page-reading and search/browse tools. |
...
- **Toolkit config** — Runtime options selected by the bootstrap installer are stored in user-level toolkit config rather than shell profiles; environment variables keep highest precedence. Runtime tools and installer/doctor flows share the same config core for schema, defaults, precedence, validation, and write behavior. See [ADR 0002](docs/adr/0002-toolkit-config-for-installer-selections.md).
...
- **Shared utility modules** — Common concerns (toolkit config semantics, browser action language, page extraction, Firecrawl Keyless behavior, content preview, output sink, CLI runner) live in `extensions/utils/` as deep modules with narrow interfaces. Tools import them; duplication is avoided.
...
- **Optional keyless cloud fallback** — The local tools (`web_search`, `web_fetch`, `web_browse`) transparently retry through the shared Firecrawl Keyless module when their local backend fails or returns nothing; three explicit `firecrawl_*` tools are also exposed. The module owns keyless-only CLI execution, graceful skip, parsing, fallback decisions, and interact cleanup. The fallback is keyless-only, never the primary path, and opt-out-able. See [ADR 0001](docs/adr/0001-firecrawl-keyless-cloud-fallback.md).
